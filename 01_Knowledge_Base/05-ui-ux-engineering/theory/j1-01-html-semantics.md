...
**Case study — semantics không chỉ là "gu code sạch":**  
Vụ kiện _Robles v. Domino's Pizza_ (Mỹ) là ví dụ hay được nhắc trong giới a11y: năm 2019, tòa phúc thẩm liên bang khẳng định website/app cũng phải tuân thủ luật chống phân biệt đối xử (ADA) như cửa hàng vật lý; Tối cao Pháp viện Mỹ từ chối xét lại, vụ án tiếp tục và Domino's thua ở cấp tòa dưới — nguyên nhân gốc: người dùng screen reader **không đặt được hàng** vì giao diện không lộ đúng cấu trúc/nhãn cho control. Semantic HTML sai không chỉ là "thiếu điểm code review" — với sản phẩm public, nó là rủi ro pháp lý và mất khách hàng thật.
...
- Chọn thẻ = gắn **nghĩa**, không phải diện mạo — CSS/class không thay được landmark.
- Bộ khung tối thiểu mọi trang: `header` → `nav` → `main` (đúng 1 cái) → `section`/`article` → `footer`.
- `<a>` = đi tới nơi khác (kể cả qua `<Link>` của framework — HTML cuối vẫn là `<a href>`); `<button>` = hành động tại chỗ, luôn khai báo `type`.
- Không rõ cấu trúc → Clarify trước, đừng "div hóa" rồi tính sau.
