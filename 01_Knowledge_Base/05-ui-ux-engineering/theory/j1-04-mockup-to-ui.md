...
Chưa cần pixel-perfect mọi breakpoint — nhưng phải **nhận biết** các con số bắt buộc và map đúng sang CSS/component.
...
## 2. Đọc mockup — những gì cần nhận ra
...
| **Thành phần** | Nút primary, input, avatar, link                | Button / Input / Card theo docs (UI library)                    |
...
## 3. Quy trình implement tối thiểu
...
Nhận biết và kiểm tra `box-sizing` + computed style trước khi tranh cãi với design.
...
**Nhận biết — công cụ đọc số từ mockup:**

| Công cụ                                   | Cho bạn gì                                                                                        |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **Figma Inspect / Dev Mode**              | Click vào layer → panel bên phải ra đúng px/rem, font, màu, khoảng cách — nguồn số chính xác nhất |
| **Bảng spec dạng chữ** (`MOCKUP_SPEC.md`) | Khi không có Figma — quy ước một "nguồn sự thật" bằng markdown, như demo mockup                   |
| **DevTools Computed / Box Model**         | Đo lại **sau khi** code — đối chiếu ngược với spec, không chỉ nhìn mắt                            |

Không có Figma/spec rõ → đó chính là dấu hiệu cần Clarify, không phải lý do để đoán số.
...
3. **Trạng thái thiếu trên mockup:** hover? empty bio? loading? (ghi nhận; chưa bắt buộc implement hết)
...
- Đọc mockup theo 5 nhóm: cấu trúc, spacing, typography, thành phần, tương tác — không nhóm nào để "nhìn mắt".
- Có bảng spec (Figma hoặc `.md`) = một nguồn sự thật; không có → Clarify trước khi code.
- Lệch 2–4px thường không phải do designer — kiểm tra `box-sizing`/border trước khi tranh cãi.
- DevTools đo lại sau khi code là bước rẻ nhất để tránh reject từ design.
