...
| Cách 24 giữa các card | `margin` hoặc gap của layout (Flex/Grid — #B)     |
...
**Case study — vì sao gần như mọi UI library đều tự reset `box-sizing`:**  
Bootstrap, Tailwind, MUI... đều nạp sẵn `*, *::before, *::after { box-sizing: border-box; }` ngay từ file CSS gốc. Lý do: `content-box` (mặc định của trình duyệt) từng gây hàng loạt bug "layout vỡ khi thêm border/padding" trong thời kỳ CSS thuần — đủ phổ biến để cả ngành coi `border-box` là **quy ước ngầm** của UI hiện đại. Khi bạn dùng lib mà không tự set lại, bạn đang **thừa hưởng** quy ước này — nhưng vẫn nên biết vì sao nó tồn tại, để khi làm việc trên CSS thuần (không lib) không quên.
...
| Công cụ     | Bản chất                                      | Nhận biết dùng khi                                              |
| ----------- | --------------------------------------------- | --------------------------------------------------------------- |
...
| Vùng                    | Hướng chọn                                                                                                                                    |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Lưới card               | `display: grid; grid-template-columns: 1fr` → đổi thành `1fr 1fr` khi đủ rộng (nhận biết 2 trạng thái; media query chi tiết luyện ở exercise) |
...
**Nhận biết — kỹ thuật layout cũ (để hiểu code legacy, không phải để chọn dùng):**

| Kỹ thuật                              | Bản chất                                         | Vì sao không nên chọn mới ở giai đoạn nền tảng     |
| ------------------------------------- | ------------------------------------------------ | -------------------------------------------------- |
| `float: left/right`                   | Vốn sinh ra để chữ "chảy" quanh ảnh báo          | Phải tự `clear`, dễ vỡ khi nội dung đổi chiều cao  |
| `table` layout                        | Dùng bảng HTML để canh cột trang                 | Sai semantics (không phải dữ liệu dạng bảng), cứng |
| `position: absolute` cho layout chính | Đặt tọa độ tuyệt đối                             | Mất luồng tự nhiên, không tự co giãn theo nội dung |
| **Flex / Grid**                       | Mô tả **ý định xếp chỗ** cho trình duyệt tự tính | Đây là hướng đúng — lý do file này tồn tại         |

Nhận ra `float`/`table` trong code cũ giúp bạn hiểu **vì sao** một trang legacy khó sửa responsive — không phải để bắt chước.
...
- Box Model: content → padding → border → margin. Ưu tiên `box-sizing: border-box` để `width` khớp mockup sau khi cộng padding/border.
- `padding` = không khí **trong** hộp; `margin` = khoảng cách **giữa** các hộp.
- Flex = xếp theo **một trục** (hàng/cột); Grid = xếp theo **ma trận hàng×cột**. Có thể lồng: Grid ngoài + Flex trong từng ô/card.
- Luôn ưu tiên `gap` trên container thay vì `margin` thủ công trên từng item con.
